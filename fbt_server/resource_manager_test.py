...
class ResourceStoreManager(object):
    ...
    @classmethod
    def extract_resource_from_db(cls, res, need_private_owner=False, file_in_dir=None):
        one_resource = dict()
    ...
    def search_resources(cls, key_word, page, sort_by, max_resources_cnt_in_page):
        ...
        if ResourceInfo.is_valid_tag(key_word):
            ...
            ret = list()
            found_file_ids = yield FileNameSearcher().query_file_ids_by_file_name(key_word)
            (start_index, end_index) = ((page - 1) * max_resources_cnt_in_page, page * max_resources_cnt_in_page)
            size = len(found_file_ids)
            index = start_index
            for file_id in found_file_ids[start_index:]:
                res = yield cls._db.all_resources.find_one({"file_id": file_id, "hidden": {"$ne": 1}, "public": 1})
                if res:
                    one_resource = cls.extract_resource_from_db(res)
                    ret.append(one_resource)
                    index += 1
    ...
    @classmethod
    @gen.coroutine
    def navigate_resources(cls, main_type, page, sort_by, max_resources_cnt_in_page,
    ...
    @classmethod
    @gen.coroutine
    def get_resources_by_file_ids(cls, hashes_list, file_size_list=None):
    ...
    @classmethod
    @gen.coroutine
    def user_upload_resource(cls, is_v4,  # user download type
                             uid,  # unique user id
                             user_name,  #user name registered
                             file_hash,  #file hash
        ...
        if need_pub_res:
            ...
            print "+++++++++++++++++++++++++++++++a publish:"+file_id+" "+str(uid)
        print "+++++++++++++++++++++++++++++++b"
        yield cls._add_resource_for_tag(file_id, tags)
    ...
    def _add_to_my_resource_list(cls, uid, file_id):
        assert (uid >= 0)
        #yield cls._db.resources_of_user.update({"uid": uid}, {"$addToSet": {"file_ids": file_id}}, True)
        print "uid %d file_id %s" % (uid, file_id)
        yield cls._db.resources_of_user.update({"uid": uid}, {"$push": {"file_ids": file_id}}, True)
        print "uid %d file_id %s OK" % (uid, file_id)
    ...
    def _add_file_in_dir_to_my_resource_list(cls, uid, file_ids):  # file_ids is a list of file_id
        ...
        #yield cls._db.resources_of_user.update({"uid": uid}, {"$addToSet": {"file_ids": {"$each": file_ids}}}, True)
    ...
    def _remove_from_my_resource_list(cls, uid, file_id):
        assert (uid >= 0)
        # yield cls._db.resources_of_user.update({"uid": uid}, {"$pull": {"file_hashes": file_id}}, True)
