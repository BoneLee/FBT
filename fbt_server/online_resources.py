...
from constant import CACHE_OWNER_OF_RESOURCE, CHANNEL_ON_OFF,CACHE_OWNER_CNT_OF_RESOURCE
...
class OnlineResources(object):
    ...
    def get_resource_of_user(cls, uid):
        return FBTUserResourceManager.instance().get_resource_of_user2(uid, cls._sync_db)
        # uid = long(uid)
    ...
    @classmethod
    def set_db_and_redis(cls, db, redis):
        cls._redis = redis
        cls._db = db

    @classmethod
    def increase_online_owners(cls, file_id, uid, main_type=None):
        not_exists = int(cls._redis.sadd(CACHE_OWNER_OF_RESOURCE + str(file_id), uid))
        if main_type is not None and not_exists:
            cls._redis.zincrby(CACHE_OWNER_CNT_OF_RESOURCE+str(main_type), file_id, 1)

    @classmethod
    def decrease_online_owners(cls, file_id, uid, main_type=None):
        exists = int(cls._redis.srem(CACHE_OWNER_OF_RESOURCE + str(file_id), uid))
        if main_type is not None and exists:
            cls._redis.zincrby(CACHE_OWNER_CNT_OF_RESOURCE+str(main_type), file_id, -1)
    ...
    def _add_online_info_to_public_res(cls, file_id, uid):
        ...
        if type_info and 'main_type' in type_info and 'public' in type_info:
            ...
            cls.increase_online_owners(file_id, uid, res_type)
    ...
    def _rm_online_info_from_res(cls, file_id, uid):
        ...
        if type_info and 'main_type' in type_info and 'public' in type_info:
            ...
            cls.decrease_online_owners(file_id, uid, res_type)
        ...
        if msg_type == msg["online"]:
            ...
            for file_id in cls.get_resource_of_user(uid):
                if cls._is_subfile(file_id):
                    ...
                    cls.increase_online_owners(subfile_id, uid)
            ...
            for file_id in cls.get_resource_of_user(uid):
                if cls._is_subfile(file_id):
                    (dir_id, subfile_id) = cls._get_subfile_id(file_id)
                    cls.decrease_online_owners(subfile_id, uid)
                    if dir_id not in dir_set:
                        dir_set.add(dir_id)
        ...
        if RedisSubHelper().is_located_myself(msg):
            ...
            if cls._redis.sismember(cls._ONLINE_UID_CACHE_KEY, uid):
                if "is_subfile" in msg and msg["is_subfile"]:
                    cls.increase_online_owners(file_id, uid)
                else:
                    cls._add_online_info_to_public_res(file_id, uid)
    ...
    def on_resource_del(cls, msg):
        if RedisSubHelper().is_located_myself(msg):
            ...
            if cls._redis.sismember(cls._ONLINE_UID_CACHE_KEY, uid):
                if "is_subfile" in msg and msg["is_subfile"]:
                    cls.decrease_online_owners(file_id, uid)
                else:
                    cls._rm_online_info_from_res(file_id, uid)
    ...
    @classmethod
    def get_online_cnt(cls, start, end, main_type):
        ret = cls._redis.zrange(CACHE_OWNER_CNT_OF_RESOURCE+str(main_type), start, end-1, desc=True, withscores=True)
        return [(file_id, int(cnt)) for (file_id, cnt) in ret]
    ...
    def get_online_resources_by_type(cls, main_type=1, page=1, max_cnt_in_page=20):
        ...
        resource_pairs = cls.get_online_cnt(start, end, main_type)
        ...
        for file_id, online_num in resource_pairs:
