...
from asyncRedis import AsyncStrictRedis as StrictRedis
from asyncRedis import AsyncConnectionPool as ConnectionPool, AsyncConnection as Connection
...
class SentinelManagedConnection(Connection):
    ...
    def connect(self):
        ...
        if self.connection_pool.is_master:
            print 'master'
            master_address = yield self.connection_pool.get_master_address()
            print master_address, 'master_address'
            self.connect_to(master_address)
        else:
            for slave in self.connection_pool.rotate_slaves():
                try:
                    raise gen.Return(self.connect_to(slave))
...
class SentinelConnectionPool(ConnectionPool):
    ...
    def get_master_address(self):
        ...
        master_address = yield self.sentinel_manager.discover_master(
            self.service_name)
        print 'sdfdsf', master_address
        if self.is_master:
            if self.master_address is None:
                ...
                self.disconnect()
        raise gen.Return(master_address)

    def rotate_slaves(self):
        print "Round-robin slave balancer"
...
class AsyncSentinel(object):
    ...
    def discover_master(self, service_name):
        ...
        for sentinel_no, sentinel in enumerate(self.sentinels):
            try:
                masters = yield sentinel.sentinel_masters()
            except ConnectionError:
                continue
            ...
            if state and self.check_master_state(state, service_name):
                ...
                raise gen.Return((state['ip'], state['port']))
    ...
    def discover_slaves(self, service_name):
        ...
        for sentinel in self.sentinels:
            try:
                slaves = yield sentinel.sentinel_slaves(service_name)
            except (ConnectionError, ResponseError):
                continue
            slaves = self.filter_slaves(slaves)
            if slaves:
                raise gen.Return(slaves)
        raise gen.Return([])
