...
.service('toaster', function ($rootScope) {
    this.pop = function (type, title, body, autoCancel) {
        this.toast = {
            type: type,
            title: title,
            body: body,
...
.constant('toasterConfig', {
					...
					'extended-time-out': 10000,    // not implemented
  ...
  return {
    ...
    link: function (scope, elm, attrs){
      ...
      function addToast (toast){
        ...
        if (mergedConfig['time-out'] > 0 && toast.autoCancel)
            setTimeout(toast, mergedConfig['time-out']);
          else if(mergedConfig['time-out'] > 0 && !toast.autoCancel)
            setTimeout(toast, mergedConfig['extended-time-out']);
