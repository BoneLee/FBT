...
'use strict';
angular.module('scselector', [])
.service('scselector', function($rootScope, $timeout) {
  return {
    show: function(title, groups, groupWidth, entityWidth, entityCallback, universityEntityCallback, submitCallback) {
      $timeout(function() {
        $rootScope.$broadcast('scselector-show', {
          title: title,
          groups: groups,
    ...
    hide: function() {
      $timeout(function() {
        $rootScope.$broadcast('scselector-hide');
      });
    }
  };
})
.directive('scselectorContainer', ['scselector', function(scselector) {
  var template = '<div id="scselector-container" class="modal fade">\
    <div class="modal-dialog">\
      <div class="modal-content">\
        ...
        <div class="modal-footer">\
          ...
          <button class="btn default primary" ng-class="{disabled: !(currentGroup && currentEntity && currentCollegeEntity)}" ng-click="submitHandler(currentEntity, currentCollegeEntity)">确定</button>\
        </div>\
      </div>\
    </div>\
  </div>';
  return {
    ...
    link: function(scope, elem, attrs) {
      ...
      scope.$on('scselector-show', function(event, d) {
        scope.title = d.title;
        ...
        scope.submitHandler = function(entity, college_entity) {
          if(d.submitCallback && typeof(d.submitCallback) == 'function') {
            ...
            $('#scselector-container').modal('hide');
          }
        };

        $('#scselector-container').modal({backdrop: false, keyboard: false});
      });
      scope.$on('scselector-hide', function() {
      });
    }
  };
}]);
